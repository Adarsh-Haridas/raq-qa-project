...
class RAGChain:
    def __init__(self,vector_store_service: VectorStoreService | None=None):
        ...
        self.chat_prompt=ChatPromptTemplate.from_template(RAG_PROMPT_TEMPLATE)
        ...
        logger.info(
            f"Rag Chain Initialized with model: {settings.llm_model}, "
    ...
    def query_with_sources(self,question: str) -> dict:
        ...
        try:
            ...
            logger.info(f"Query processed with {len(sources_docs)} sources")
            return {
                "answer":answer,
                "sources":sources
            }
    ...
    async def aquery_with_evaluation(self,question: str, include_sources: bool=True) ->dict:
        ...
        logger.info(f"Processing query with evaluation: {question[:150]}...")
        ...
        try:
            ...
            logger.error(f"Error in query with elevator: {e}")
            raise
